...
function rssfeedclass(adapter) {
    ...
    this.initCmdStates = function(callback) {   
        ...
        this.getState(stateTemplate.name, this.RssFeedPath,false,function (err, state) {
            if (state==null) {
                stateTemplate.def = "{}";
                this.createState(stateTemplate,this.RssFeedPath,false,callback);
            } else {
                if (state.val == null || state.val=="" || state.val=="[]") {
                        ...
                        if (!err) {
                            ...
                            this.getState(item.name, item.cat,false,function (err, state) {
                                if (state==null) {
                                    ...
                                    this.log.debug('getData finished');                                
                                }
                            }.bind(this));
                        } else {
    ...
    this.request = function(url,request_cb) {
        var that = this;
        ...
        transformEncoding.on('error',function(err){
            transformEncoding.that.log.debug ('transform error');          
            transformEncoding.that.log.error (err.toString());          
        });
        var feedparser = new FeedParser();
        feedparser.on ("error", function (err) {
            that.log.debug('feedparser error');
            that.log.error (err.toString());
            request_cb(err);
		});
        feedparser.on('readable', function () {
            that.log.debug('feedparser readable');
        ...
        try {
           ...
           // Anweisungen für jeden Fehler
            this.log.debug('request error: ' + e.toString());
        }
