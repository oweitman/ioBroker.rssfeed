...
function rssfeedclass(adapter) {
        ...
        this.getState(stateTemplate.name, this.RssFeedPath, false, (err, state) => {
            ...
            rss.forEach((item, index, rss) => {
                ...
                if (lastrequest + item.refresh * 60 * 1000 < Date.now()) {
                    ...
                    this.logdebug(`getData skip: ${item.name}`);
                }
            });
        });
    };
    ...
    this.request = function (url, maxarticles, request_cb) {
        ...
        // @ts-expect-error additional var
        feedparser.url = url;
        // @ts-expect-error additional var
        ...
        try {
            ...
            if (this.adapter.config.useragent) {
                headers['User-Agent'] = this.adapter.config.useragent;
            }
        ...
        this.adapter.getObject(name, (err, obj) => {
            ...
            if (!obj) {
                callback
                    ...
                    : this.adapter.setObjectNotExists(name, newobj); // jshint ignore:line
            } else {
                if (callback) {
                    callback();
                }
    ...
    this.setTimeout = function (id, callback, time) {
        ...
        this.observers[id] = this.adapter.setTimeout(callback.bind(this), time);
    };
