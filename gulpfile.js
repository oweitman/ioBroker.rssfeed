...
gulp.task("translate", async function (done) {
    ...
    if (iopackage && iopackage.common) {
        ...
        if (fs.existsSync("./admin/i18n/en.json")) {
            let enTranslations = require("./admin/i18n/en.json");
            for (let l in languages) {
                console.log("Translate Text: " + l);
                let existing = {};
                if (fs.existsSync("./admin/i18n/" + l + ".json")) {
                    existing = require("./admin/i18n/" + l + ".json");
                }
                for (let t in enTranslations) {
                    if (!existing[t]) {
                ...
                fs.writeFileSync("./admin/i18n/" + l + ".json", JSON.stringify(existing, null, 4));
            }
        }
    }
    fs.writeFileSync("io-package.json", JSON.stringify(iopackage, null, 4));
